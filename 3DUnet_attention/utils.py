...
def crop_brain_region(im, gt, with_gt=True):
    ...
    if with_gt:
        label = load_nifty_volume_as_array(gt, with_header=False, with_gt=True)
        # label from 1 2 3 to 2 4 1
        if config.DATASET == 'iseg':
            # for iSeg-2017 [10, 150, 250] -> [1, 2, 3]
            # label = change_brats_iseg_label(label, [10, 150, 250], [1, 2, 3])
            label = change_brats_iseg_label(label, [1, 2, 3], [2, 4, 1])
            # label = change_brats_iseg_label(label, [10, 150, 250], [2, 4, 1])
...
#     del mask_volume, convert_volume
#     return out_volume
...
def load_nifty_volume_as_array(filename, with_header = False, with_gt = False):
    """
    ...
    if config.DATASET == "iseg" and with_gt == True:
        data = change_brats_iseg_label(data, [10, 150, 250], [1, 2, 3])
    del img
    if(with_header):
...
def change_brats_iseg_label(source_seg, source, target):
    target_seg = np.zeros_like(source_seg)
    for i in range(3):#len(source)):
        target_seg[source_seg == source[i]] = target[i]
    del source_seg
    return target_seg
